...
import { Camera, Video, FileText, Smile, MoreHorizontal, X, Upload } from "lucide-react";
  ...
  const handleFileUpload = (event, fileType) => {
    ...
    files.forEach(file => {
      ...
      if (file.size > maxSize) {
        alert(`File "${file.name}" is too large. Please select ${fileType}s smaller than ${maxSizeText}.`);
        return;
      }
      ...
      if (fileType === 'photo') {
        ...
        if (file.size <= 500 * 1024) { // 500KB limit for documents
          const reader = new FileReader();
          reader.onload = (e) => {
            const newFile = {
              id: Date.now() + Math.random(),
      ...
      try {
        ...
        const newPost = {
          ...
          attachments: attachedFiles.map(file => ({
            ...
            previewText: file.type === 'video' ? '🎥 Video file (session only)' : 
                        file.type === 'document' && !file.storedLocally ? '📄 Document file (not stored)' : 
                        file.type === 'document' ? '📄 Document file' : null
          }))
        };
        ...
        try {
          ...
          if (dataSize > 4.5 * 1024 * 1024) { // 4.5MB limit to be safe
            // Remove oldest posts to make space
            const reducedPosts = existingPosts.slice(0, Math.max(1, existingPosts.length - 10));
          ...
          if (quotaError.name === 'QuotaExceededError') {
            ...
            const reducedPosts = existingPosts.slice(0, Math.max(1, Math.floor(existingPosts.length / 2)));
            try {
              localStorage.setItem('feedPosts', JSON.stringify(reducedPosts));
              alert('Storage limit reached. Removed many old posts to make space.');
            } catch (stillError) {
              // If still failing, clear all posts except the new one
              localStorage.setItem('feedPosts', JSON.stringify([newPost]));
              alert('Storage severely limited. Cleared old posts to save your new post.');
            }
          } else {
            throw quotaError;
          }
