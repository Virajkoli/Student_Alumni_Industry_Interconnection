...
class GitHubAuthService {
  ...
  async handleCallback(code) {
    try {
      ...
      const response = await fetch(`${this.apiBaseUrl}/api/auth/github/callback?code=${code}`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to exchange GitHub code for token: ${response.status} ${errorText}`);
      }
