...
async function addGoogleFields() {
  try {
    ...
    console.log('Adding googleId to Students table...');
    await sequelize.query(`
      ALTER TABLE "Students" ADD COLUMN "googleId" VARCHAR(255) UNIQUE;
    `).catch(err => {
      if (err.message.includes('already exists')) {
        console.log('⚠️  googleId column already exists in Students table');
      } else {
        throw err;
      }
    });
    ...
    console.log('Adding googleId to Colleges table...');
    await sequelize.query(`
      ALTER TABLE "Colleges" ADD COLUMN "googleId" VARCHAR(255) UNIQUE;
    `).catch(err => {
      if (err.message.includes('already exists')) {
        console.log('⚠️  googleId column already exists in Colleges table');
      } else {
        throw err;
      }
    });
