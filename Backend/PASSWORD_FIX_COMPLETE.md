...
- `google_id` → `googleId` (uses field mapping to `google_id`)
...
newUser = await Student.create({
  ...
  google_id: googleId, // ❌ Wrong field name
...
newUser = await Student.create({
  ...
  first_name: firstName,
  googleId: googleId, // ✅ Correct field name with mapping
...
### Field Mapping:
- **JavaScript code**: Uses `googleId` and `imageUrl`
