...
export default function GitHubSignInButton({ isSignUp, onSuccess, onError }) {
  ...
  const handleGitHubSignIn = async () => {
    ...
    try {
      ...
      try {
        ...
        onError("Browser storage is not available. Please enable cookies and try again.");
        setIsLoading(false);
        return;
      }
      ...
      setIsLoading(false);
      const errorMessage = `GitHub authentication failed: ${error.message || 'Unknown error'}`;
  ...
  return (
    ...
    >
      ...
      {isLoading ? "Processing..." : (isSignUp ? "Sign up with GitHub" : "Sign in with GitHub")}
    </button>
  );
}
