...
async function checkGoogleColumns() {
  try {
    ...
    const studentsHasImageUrl = studentsColumns.some(col => col.column_name === 'image_url');
    const collegesHasImageUrl = collegesColumns.some(col => col.column_name === 'image_url');
    
    console.log('\n🔧 Column Status:');
    console.log(`Students table has image_url: ${studentsHasImageUrl ? '✅ YES' : '❌ NO'}`);
    console.log(`Colleges table has image_url: ${collegesHasImageUrl ? '✅ YES' : '❌ NO'}`);
    
    // Add missing image_url columns if needed
    if (!studentsHasImageUrl) {
      console.log('\n➕ Adding image_url column to Students table...');
      try {
        await sequelize.query('ALTER TABLE "Students" ADD COLUMN "image_url" VARCHAR(500);');
    ...
    if (!collegesHasImageUrl) {
      ...
      try {
        await sequelize.query('ALTER TABLE "Colleges" ADD COLUMN "image_url" VARCHAR(500);');
