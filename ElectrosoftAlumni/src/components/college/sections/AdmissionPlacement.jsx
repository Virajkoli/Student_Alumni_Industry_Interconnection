...
const AdmissionPlacement = ({ data, onEdit }) => {
  if (!data) return null;
  // Heuristically detect if this is admission or placement data
  const isAdmission = !!data.eligibility;
  const isPlacement = !!data.highlights;
  return (
    <div className="bg-white rounded-xl shadow p-6 mb-6">
      <div className="flex justify-between items-start mb-2">
        <h2 className="text-xl font-bold text-blue-900 mb-4">
          {isAdmission ? "Admission" : isPlacement ? "Placement" : "Admission & Placement"}
        </h2>
        <button
      ...
      {isPlacement && (
        <>
          <div className="mb-6">
            <h3 className="font-semibold text-blue-800 mb-2 text-lg">Placement Highlights</h3>
            <ul className="list-disc list-inside text-gray-700 space-y-2 text-base leading-7">
          ...
          <div className="mb-4">
            ...
            <ul className="list-disc list-inside text-gray-700 space-y-2 text-base leading-7">
              {data.internships && data.internships.map((item, idx) => (
                <li key={idx}>{item}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="list-disc list-inside text-gray-700 space-y-2 text-base leading-7">
              {data.support && data.support.map((item, idx) => (
                <li key={idx}>{item}</li>
              ))}
            </ul>
          </div>
...
export default AdmissionPlacement;
