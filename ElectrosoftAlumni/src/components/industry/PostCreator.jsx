  ...
  const handlePost = () => {
    if (postText.trim() || attachedFiles.length > 0) {
      try {
        ...
        // Get existing posts from localStorage (use industry-specific key)
        ...
        alert("Industry post created successfully!");

        // Trigger a custom event to notify FeedArea of new post
        window.dispatchEvent(new Event("newIndustryPost"));
      } catch (error) {
        console.error("Error creating industry post:", error);
